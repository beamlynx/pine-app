...
export const CHANGELOG: ChangelogVersion[] = [
{
  ...
  },
  {
    ...
    changed: [
      ...
      {
        description: 'Support for `?data=<encoded-object-with-expression>` URL parameter which is json encoded object containing the expression'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    changed: [
      {
        description: 'Update model is not shown by default. It is shown when the inspect icon is clicked'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    changed: [
      {
        description: 'The connection monitor is moved together with the other menu items in the settings'
      },
      {
  ...
  },
  {
    ...
    changed: [
      {
        description: 'The expression is prettified when a table expression is selected from the autocompletion'
      }
    ],
    fixed: [
      {
      ...
      {
        description: 'Position of the &apos;Download CSV&apos; button in compact mode was overlapping with the &apos;Run&apos; button'
      },
      {
        description: 'Autocomplete was not showing if opened too fast. Now we always have a backup no-op completion i.e. `Nothing found`'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    fixed: [
      {
        description: 'Focus on the input when the Escape key is pressed. This wasn&apos;t working if the mouse was used to click on other components of the UI'
      }
    ]
  },
  {
    ...
    added: [
      ...
      {
        description: 'The id column in the results are clickable. This adds a where condition and limits the results to the row clicked'
      },
      {
        description: 'For a screen size less than 1200px (i.e. lg), the we update the layout accordingly. Instead of showing the SQL query, the main view is shown'
      }
    ],
    fixed: [
      {
      ...
      {
        description: 'The graph is rendered for each table being evaluated when doing recursive deletes'
      }
    ],
    changed: [
      {
  ...
  },
  {
    ...
    fixed: [
      {
        description: 'The delete queries use the correct column name i.e. column used in the previous join than the first column of the table'
      }
    ]
  },
  {
    ...
    added: [
      {
        description: 'If the pine server isn&apos;t running, then the correct version is shown instead of `latest`'
      }
    ]
  },
  {
    ...
    added: [
      {
        description: 'Clicking on a suggested column (select or order) in the selected node updates the expression'
      }
    ],
    changed: [
      {
  ...
  },
  {
    ...
    added: [
      ...
      {
        description: 'User preferences using local storage: sidebar width is supported to begin with'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    fixed: [
      ...
      {
        description: 'Sidebar width for smaller screens i.e. adjust the width when the dev console is opened'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    fixed: [
      {
        description: 'When adding a pipe `|`, the expression was always being prettified. This wasn&apos;t allowing for adding pipes in the middle of the expression'
      },
      {
        description: 'When a candidate is selected in the graph, entering a non-printable character was entering the name of that character to the expression'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    breaking: [
      {
        description: '`Tab` to focus on the graph. `Esc` or `Shift + Tab` to focus back on the input'
      },
      {
      ...
      {
        description: 'When focused on the graph, `Enter` selects the current candidate. Any other character brings you back to the input'
      }
    ]
  },
  {
  ...
  },
  {
    ...
    fixed: [
      ...
      {
        description: 'In some cases, clicking on a cell would duplicate the rows / throw an error on moving away from the cell'
      }
    ]
  },
  {
